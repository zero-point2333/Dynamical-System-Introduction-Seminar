...
\begin{document}
    ...
    \begin{example}[圆的旋转]
        ...
        显然，当 $\alpha\in\Q$ 时，所有点都是周期点，而 $\alpha\notin \Q$ 时，每个点的前向轨道都稠密（请证明）. 前向轨道的稠密性是一个典型的拓扑性质，这就引出了我们的第一个刻画.
    \end{example}

    \begin{definition}[拓扑传递、极小的轨道定义]
        $f:X\to X$ 是连续函数，若 $\exists\; x\in X$ 使得前向轨道 $\cO_f(x)={\{f^n(x)\}}_{n\in\Z}$ 在 $X$ 中稠密，则称 $f$ 拓扑传递；若 $\forall\; x\in X,\; \overline{\cO_f(x)}=X$，则称 $f$ 极小.
    \end{definition}
    \begin{remark}
        显然，极小的映射一定是拓扑传递的.
    \end{remark}
    ...
    \begin{definition}[拓扑传递的拓扑定义]
        设 $X$ 是局部紧可分度量空间，$f:X\to X$ 是连续映射，若对任意非空开集 $U,V\subset X$ 都存在整数 $N=N(U,V)$，使得 $f^{N}(U)\cap V\neq\emptyset$，则称 $f$ 拓扑传递.
    \end{definition}
    \begin{proof}[相合性证明]
        轨道定义 $\Rightarrow$ 拓扑定义是显然的.

        对于 $\Leftarrow$ 方向，注意到局部紧可分度量空间有可数邻域基（也即存在一个开集族 $\{U_i\}$ 使得 $\forall\;x\in X$，任取 $x$ 的开邻域 $U$，则存在 $i$ 使得 $x\in U_i\subset U$）. 更进一步的，我们可以设 $\{U_i\}$ 是预紧开集族. 由于拓扑定义，存在 $N_1$ 使得 $f^{N_1}(U_1)\cap U_2\neq\emptyset$，于是设非空开集 $V_1$ 满足 $\overline{V}_1\subset U_1\cap f^{-N_1}(U_2)$（这里我们使用度量空间的拓扑基来保证 $V_1$ 的存在性），由于 $U_1$ 预紧则 $V_1$ 预紧. 类似的，存在 $N_2$ 使得 $f^{N_2}(V_1)\cap U_3\neq\emptyset$，于是存在非空预紧开集 $V_2$ 使得 $\overline{V_2}\subset V_1\cap f^{-N_2}(U_3)$，于是以此类推得到 $\{V_n\}$. 设闭集套 $V=\bigcap\limits_{n=1}^{\infty}\overline{V}_n=\bigcap\limits_{n=1}^{\infty}V_n$，由于 $\overline{V}_1$ 紧而非空，设 $x\in V$，则 $x$ 轨道稠密（只要经过所有 $U_i$ 即可）.
    \end{proof}
