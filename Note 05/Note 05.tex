...
\begin{document}
    ...
    \begin{remark}
        \begin{itemize}
            \item 显然严格不变集是不变集（这里稍微放宽一下不变集，也即要求 $f(A)\subset A$）.
    ...
    \begin{proposition}
        若 $f:X\to X$ 是紧度量概率测度空间上的唯一遍历的连续函数，则任意 $\varphi\in C(X)$，时间平均 $\frac{1}{n}\sum\limits_{k=0}^{n-1}\varphi(f^k(x))$ 一致收敛到常数.
    \end{proposition}
    \begin{proof}
    ...
    \end{corollary}
    \begin{proof}
        取 $\bar{\varphi}_k\geq \chi_U$ 连续，且 $\int_{X}\bar{\varphi}_k d\mu<\mu(U)+\frac{1}{2^n}$，于是 $\frac{1}{n}\sum\limits_{k=0}^{n-1}\chi_U(f^k(x))\leq \frac{1}{n}\sum\limits_{k=0}^{n-1}\bar{\varphi}_k(f^k(x))\stackrel{n\to\infty}{\longrightarrow}\bar{\varphi}_{k,T}(x)=\int_{X}\bar{\varphi}_{k,T}(x)dx=\int_{X}\bar{\varphi}_k(x)dx<\mu(U)+\frac{1}{2^k}$，令 $k\to 0$ 即知 $\varlimsup\limits_{n\to\infty}\frac{1}{n}\sum\limits_{k=0}^{n-1}\chi_U(f^k(x))\leq \mu(U)$. 同理取 $\underline{\varphi}_k\leq \chi_U$ 也有 $\varliminf\limits_{n\to\infty}\frac{1}{n}\sum\limits_{k=0}^{n-1}\chi_U(f^k(x))\geq \mu(U)$，于是即证.
    \end{proof}
    ...
    \begin{example}
        ...
        \end{bmatrix}$），则 $L$ 与 $\sigma_A$ 度量同构（环面取 Lebesgue 测度），从而由于 $\sigma_A$ 混合，则 $L$ 混合. 更一般的，我们有如下结论：
    \end{example}

    \begin{proposition}
        二维环面上的任何线性双曲自同构在 Lebesgue 测度下都是混合的.
    \end{proposition}
    \begin{proof}
